...
export function renderWeekView(ctx) {
  ...
  days.forEach((dayName, di) => {
    ...
    html += `<div class="week-day ${isToday ? 'today' : ''} ${collapsed ? 'collapsed' : ''}" data-week-drop-day="${escapeHtml(dayName)}">`;
      ...
      day.sections.forEach((section, sIdx) => {
        ...
        section.items.forEach((item, iIdx) => {
          ...
          html += `<div class="${cls}" data-task-id="${item.id}" data-action="toggle" data-id="${item.id}"
            draggable="true" data-dragstart-action="taskDragStart" data-drag-id="${item.id}" data-drag-day="${escapeHtml(dayName)}" data-drag-si="${sIdx}" data-drag-ii="${iIdx}"
